...
export async function GET() {
  try {
    ...
    // Check if test market stand already exists
    let stand = await prisma.marketStand.findFirst({
      where: { userId: user.id, name: "Test Farm Stand" }
    })

    // Create test market stand only if it doesn't exist
    if (!stand) {
      stand = await prisma.marketStand.create({
        data: {
    ...
    const existingProducts = await prisma.product.findMany({
      where: { marketStandId: stand.id }
    })

    let products = existingProducts
    
    // Create test products only if they don't exist
